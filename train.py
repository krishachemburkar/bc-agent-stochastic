...
# from loaded_gaussian_policy import LoadedGaussianPolicy  # your class file

# # path to the expert .pkl file

# # def expert_policy(state):
# #     with open("HalfCheetah.pkl", "rb") as f:
# #         expert = pickle.load(f)
# #     print(expert.keys())
# #     model = expert['GaussianPolicy']  # this is the actual policy
# #     print(model.keys())
# #     print(model['out']['AffineLayer'])
...
#         expert.eval()  # make sure it’s in evaluation mode
...
#             # Rollout episodes with current policy
...
#             # Query expert for correct actions
#             expert_actions = [expert(s) for s in collected_states]
#             print(type(expert_actions))
#             print(type(expert_actions[0]))
#             # Aggregate new data into dataset
#             observations = np.concatenate([observations, np.array(collected_states)], axis=0)
#             actions = np.concatenate([observations, np.array([a.cpu().detach().numpy() for a in expert_actions])], axis=0)
#             observations_norm = (observations - obs_mean) / obs_std
...
def train_loop(dataset, dagger=False, env_name="HalfCheetah-v4",
    ...
    if dagger:
        ...
        for iter_idx in range(dagger_iterations):
            ...
            # convert expert actions to numpy — squeeze removes extra dims
            expert_actions_np = np.array([
                a.cpu().detach().squeeze().numpy() for a in expert_actions
            ])  # shape: (N, act_dim)

            new_obs = np.array(collected_states)  # shape: (N, obs_dim)

            # BUG FIX 1: was concatenating observations into actions by mistake
            observations = np.concatenate([observations, new_obs], axis=0)
            actions = np.concatenate([actions, expert_actions_np], axis=0)  # ← was [observations, ...] before

            # BUG FIX 2: update normalization stats with new data
            obs_mean = np.mean(observations, axis=0)
            obs_std = np.std(observations, axis=0) + 1e-8
